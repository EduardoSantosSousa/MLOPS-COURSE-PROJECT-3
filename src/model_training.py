...
class ModelTraining:
    ...
    def __init__(self, feature_store:RedisFeatureStore, model_save_path = MODEL_DIR):
        ...
        logger.info("Model Training Initioalizing.....")

    def load_data_from_redis(self, entity_ids):
        try:
            logger.info("Extracting from Radis")

            data=[]
            for entity_id in entity_ids:
                features = self.feature_store.get_features(entity_id=entity_id)
                if features:
    ...
    def train_and_evaluate(self, X_train, y_train, X_test, y_test):
        try:
            best_rf = self.hyperparamter_tuning(X_train=X_train, y_train=y_train)
